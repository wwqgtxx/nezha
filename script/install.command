...
install_agent() {
    ...
    local version=$(curl -m 10 -sL "https://api.github.com/repos/wwqgtxx/nezha-agent/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://gitee.com/api/v5/repos/wwqgtxx/nezha-agent/releases/latest" | awk -F '"' '{for(i=1;i<=NF;i++){if($i=="tag_name"){print $(i+2)}}}')
    fi
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://fastly.jsdelivr.net/gh/wwqgtxx/nezha-agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/nezhahq\/agent@/v/g')
    fi
    if [ ! -n "$version" ]; then
        version=$(curl -m 10 -sL "https://gcore.jsdelivr.net/gh/wwqgtxx/nezha-agent/" | grep "option\.value" | awk -F "'" '{print $2}' | sed 's/nezhahq\/agent@/v/g')
    fi

    if [ ! -n "$version" ]; then
        echo -e "Fail to obtaine agent version, please check if the network can link https://api.github.com/repos/wwqgtxx/nezha-agent/releases/latest"
        return 0
    else
    ...
    if [[ -z $CN ]]; then
        NZ_AGENT_URL="https://${GITHUB_URL}/wwqgtxx/nezha-agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    else
        NZ_AGENT_URL="https://${GITHUB_URL}/wwqgtxx/nezha-agent/releases/download/${version}/nezha-agent_darwin_${os_arch}.zip"
    fi
